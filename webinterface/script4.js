/***    ______      ___*   / ____/___  /   | _____________  __________*  / / __/ __ \/ /| |/ ___/ ___/ _ \/ ___/ ___/* / /_/ / /_/ / ___ / /__/ /__/  __(__  |__  )* \____/\____/_/  |_\___/\___/\___/____/____/** The MIT License (MIT)* Copyright (c) 2009-2018 Gerardo Orellana <hello @ goaccess.io>*/ "use strict";
function truncate(text, width) {
    text.each(function () {
        var parent = this.parentNode,
            $d3parent = d3.select(parent);
...
function AreaChart(dualYaxis) {
    ...
    var xAxis = d3.svg
        ...
        .tickFormat(function (d) {
            if (format.x) return GoAccess.Util.fmtValue(d, format.x);
            return d;
        });
    var yAxis0 = d3.svg
        .axis()
    ...
    var yAxis1 = d3.svg
        ...
        .orient("right")
        .tickFormat(function (d) {
            if (format.y1) return GoAccess.Util.fmtValue(d, format.y1);
            return d3.format(".2s")(d);
        });
    var xGrid = d3.svg.axis().scale(xScale).orient("bottom");
    var yGrid = d3.svg.axis().scale(yScale0).orient("left");
    var area0 = d3.svg.area().interpolate("monotone").x(X).y(Y0);
    var area1 = d3.svg.area().interpolate("monotone").x(X).y(Y1);
    var line0 = d3.svg.line().interpolate("monotone").x(X).y(Y0);
    var line1 = d3.svg.line().interpolate("monotone").x(X).y(Y1);
    function X(d) {
        return xScale(d[0]);
    }
    function Y0(d) {
    ...
    function getXTicks(data) {
        if (data.length < nTicks) return xScale.domain();
    ...
    function toggleOpacity(ele, op) {
        d3.select(ele.parentNode)
            .selectAll("." + (ele.getAttribute("data-yaxis") == "y0" ? "y1" : "y0"))
            .attr("style", op);
    }
    function setLegendLabels(svg) {
        ...
        text.enter()
            ...
            .attr("y", height - 6);
        text.attr("x", width / 2 - 85).text(labels.y0);
        if (!dualYaxis) return;
        ...
        text.enter()
            ...
            .attr("y", height - 6);
        text.attr("x", width / 2 + 15).text(labels.y1);
    }
    function setAxisLabels(svg) {
        svg.selectAll("text.axis-label.y0").data([null]).enter().append("text").attr("class", "axis-label y0").attr("y", 10).attr("x", 53).text(labels.y0);
        if (!dualYaxis) return;
        var tEnter = svg.selectAll("text.axis-label.y1").data([null]);
        tEnter.enter().append("text").attr("class", "axis-label y1").attr("y", 10).text(labels.y1);
        dualYaxis && tEnter.attr("x", width - 25);
    }
    function createSkeleton(svg) {
        var gEnter = svg.enter().append("svg").append("g");
        gEnter.append("g").attr("class", "line line0 y0");
        dualYaxis && gEnter.append("g").attr("class", "line line1 y1");
        gEnter.append("g").attr("class", "area area0 y0");
        dualYaxis && gEnter.append("g").attr("class", "area area1 y1");
        gEnter.append("g").attr("class", "points y0");
        dualYaxis && gEnter.append("g").attr("class", "points y1");
        gEnter.append("g").attr("class", "x grid");
        gEnter.append("g").attr("class", "y grid");
        gEnter.append("g").attr("class", "x axis");
        gEnter.append("g").attr("class", "y0 axis");
        dualYaxis && gEnter.append("g").attr("class", "y1 axis");
        gEnter.append("g").attr("class", "rects");
        setAxisLabels(svg);
        setLegendLabels(svg);
        gEnter.append("line").attr("y2", innerH()).attr("y1", 0).attr("class", "indicator");
    }
    function pathLen(d) {
    ...
    function addArea(g, data, cb, cName) {
        var area = g
            ...
            .selectAll("path." + cName)
            .data([data]);
        area.enter().append("svg:path").attr("class", cName);
        area.attr("d", cb);
        area.exit().remove();
    }
    ...
    function addPoints(g, data) {
        ...
        points.enter().append("svg:circle").attr("r", radius).attr("class", "point");
        points
            .attr("cx", function (d) {
                return xScale(d[0]);
            })
    ...
    function addAxis(g, data) {
        ...
        g.select(".y0.axis").call(yAxis0.tickValues(getYTicks(yScale0)));
        if (!dualYaxis) return;
        g.select(".y1.axis")
    ...
    function addGrid(g, data) {
        g.select(".x.grid")
            .attr("transform", "translate(0," + yScale0.range()[0] + ")")
            .call(xGrid.tickValues(getXTicks(data)).tickSize(-innerH(), 0, 0).tickFormat(""));
        g.select(".y.grid").call(yGrid.tickValues(getYTicks(yScale0)).tickSize(-innerW(), 0, 0).tickFormat(""));
    }
    ...
    function addRects(selection, g, data) {
        ...
        rects.enter().append("svg:rect").attr("height", innerH()).attr("class", "point");
        rects
...
function BarChart(dualYaxis) {
    ...
    function createSkeleton(svg) {
        ...
        gEnter.append("g").attr("class", "y0 axis");
        dualYaxis && gEnter.append("g").attr("class", "y1 axis");
        gEnter.append("g").attr("class", "bars y0");
        dualYaxis && gEnter.append("g").attr("class", "bars y1");
        gEnter.append("g").attr("class", "rects");
        setAxisLabels(svg);
        setLegendLabels(svg);
        gEnter.append("line").attr("y2", innerH()).attr("y1", 0).attr("class", "indicator");
    ...
    function addBars(g, data) {
        ...
        bars.attr("width", xScale.rangeBand() / 2)
            .attr("x", function (d) {
                return xScale(d[0]);
            })
            .transition()
